...
template <typename T> struct Trsm;

template <> struct Trsm<float> {
  typedef float Scalar;
  static cublasStatus_t call32(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int m, int n, const float *alpha, const float *a, int lda, float *b, int ldb) {
    return cublasStrsm_v2(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
  static cublasStatus_t call64(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int64_t m, int64_t n, const float *alpha, const float *a, int64_t lda, float *b, int64_t ldb) {
    return cublasStrsm_v2_64(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
};

template <> struct Trsm<double> {
  typedef double Scalar;
  static cublasStatus_t call32(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int m, int n, const double *alpha, const double *a, int lda, double *b, int ldb) {
    return cublasDtrsm_v2(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
  static cublasStatus_t call64(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int64_t m, int64_t n, const double *alpha, const double *a, int64_t lda, double *b, int64_t ldb) {
    return cublasDtrsm_v2_64(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
};

template <> struct Trsm<cuComplex> {
  typedef cuComplex Scalar;
  static cublasStatus_t call32(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int m, int n, const cuComplex *alpha, const cuComplex *a, int lda, cuComplex *b, int ldb) {
    return cublasCtrsm_v2(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
  static cublasStatus_t call64(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int64_t m, int64_t n, const cuComplex *alpha, const cuComplex *a, int64_t lda, cuComplex *b, int64_t ldb) {
    return cublasCtrsm_v2_64(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
};

template <> struct Trsm<cuDoubleComplex> {
  typedef cuDoubleComplex Scalar;
  static cublasStatus_t call32(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int m, int n, const cuDoubleComplex *alpha, const cuDoubleComplex *a, int lda, cuDoubleComplex *b, int ldb) {
    return cublasZtrsm_v2(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
  static cublasStatus_t call64(cublasHandle_t h, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int64_t m, int64_t n, const cuDoubleComplex *alpha, const cuDoubleComplex *a, int64_t lda, cuDoubleComplex *b, int64_t ldb) {
    return cublasZtrsm_v2_64(h, side, uplo, trans, diag, m, n, alpha, a, lda, b, ldb);
  }
};
...
template <typename T>
static void fill_solution(std::vector<T> &x, int m, int n, int ldx) {
  for (int col = 0; col < n; ++col) {
    for (int row = 0; row < ldx; ++row) {
      x[row + col * ldx] = row < m ? Ops<T>::value(0.25f * static_cast<float>(2 * col + row + 1), 0.125f * static_cast<float>(row - col)) : Ops<T>::value(-200.0f - static_cast<float>(row + col), 0.0f);
    }
  }
}

template <typename T>
static void fill_rhs(std::vector<T> &b, const std::vector<T> &a, const std::vector<T> &x, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, int m, int n, int lda, int ldx, int ldb, T alpha) {
  for (int col = 0; col < n; ++col) {
    for (int row = 0; row < ldb; ++row) {
      if (row >= m) {
        b[row + col * ldb] = Ops<T>::value(-300.0f - static_cast<float>(row + col), 0.0f);
        continue;
      }
      T sum = Ops<T>::zero();
      if (side == CUBLAS_SIDE_LEFT) {
        for (int inner = 0; inner < m; ++inner) {
          sum = Ops<T>::add(sum, Ops<T>::mul(op_a(a, uplo, trans, diag, lda, row, inner), x[inner + col * ldx]));
        }
      } else {
        for (int inner = 0; inner < n; ++inner) {
          sum = Ops<T>::add(sum, Ops<T>::mul(x[row + inner * ldx], op_a(a, uplo, trans, diag, lda, inner, col)));
        }
      }
      b[row + col * ldb] = Ops<T>::div(sum, alpha);
    }
  }
}
...
static void run_case(cublasHandle_t handle, const char *name, cublasSideMode_t side, cublasFillMode_t uplo, cublasOperation_t trans, cublasDiagType_t diag, bool use_64, bool device_scalar) {
  ...
  std::vector<T> x(ldb * n);
  std::vector<T> b(ldb * n);
  fill_a(a, uplo, adim, lda);
  fill_solution(x, m, n, ldb);
  const typename Trsm<T>::Scalar alpha = Ops<T>::alpha();
  fill_rhs(b, a, x, side, uplo, trans, diag, m, n, lda, ldb, ldb, alpha);

  T *d_a = nullptr;
  T *d_b = nullptr;
  typename Trsm<T>::Scalar *d_alpha = nullptr;
  ...
  const typename Trsm<T>::Scalar *alpha_arg = &alpha;
  if (device_scalar) {
    CHECK_CUDA(cudaMalloc(&d_alpha, sizeof(typename Trsm<T>::Scalar)));
    CHECK_CUDA(cudaMemcpy(d_alpha, &alpha, sizeof(typename Trsm<T>::Scalar), cudaMemcpyHostToDevice));
    alpha_arg = d_alpha;
  ...
  if (use_64) {
    CHECK_CUBLAS(Trsm<T>::call64(handle, side, uplo, trans, diag, m, n, alpha_arg, d_a, lda, d_b, ldb));
  } else {
    CHECK_CUBLAS(Trsm<T>::call32(handle, side, uplo, trans, diag, m, n, alpha_arg, d_a, lda, d_b, ldb));
  }
  CHECK_CUBLAS(cublasSetPointerMode(handle, CUBLAS_POINTER_MODE_HOST));
  ...
  for (int col = 0; col < n; ++col) {
    for (int row = 0; row < m; ++row) {
      if (!Ops<T>::near(out[row + col * ldb], x[row + col * ldb])) {
...
int main() {
  ...
  run_type<float>(handle, "strsm", real_ops, 2);
  run_type<double>(handle, "dtrsm", real_ops, 2);
  run_type<cuComplex>(handle, "ctrsm", complex_ops, 3);
  run_type<cuDoubleComplex>(handle, "ztrsm", complex_ops, 3);

...
template <typename T> struct Trsv;

template <> struct Trsv<float> {
  ...
  static cublasStatus_t call64(cublasHandle_t handle, cublasFillMode_t uplo,
    ...
    return cublasStrsv_v2_64(handle, uplo, trans, diag, n, a, lda, x, incx);
  }
};

template <> struct Trsv<double> {
  static cublasStatus_t call32(cublasHandle_t handle, cublasFillMode_t uplo,
                               cublasOperation_t trans, cublasDiagType_t diag,
  ...
  static cublasStatus_t call64(cublasHandle_t handle, cublasFillMode_t uplo,
    ...
    return cublasDtrsv_v2_64(handle, uplo, trans, diag, n, a, lda, x, incx);
  }
};

template <> struct Trsv<cuComplex> {
  static cublasStatus_t call32(cublasHandle_t handle, cublasFillMode_t uplo,
                               cublasOperation_t trans, cublasDiagType_t diag,
  ...
  static cublasStatus_t call64(cublasHandle_t handle, cublasFillMode_t uplo,
    ...
    return cublasCtrsv_v2_64(handle, uplo, trans, diag, n, a, lda, x, incx);
  }
};

template <> struct Trsv<cuDoubleComplex> {
  static cublasStatus_t call32(cublasHandle_t handle, cublasFillMode_t uplo,
                               cublasOperation_t trans, cublasDiagType_t diag,
  ...
  static cublasStatus_t call64(cublasHandle_t handle, cublasFillMode_t uplo,
    ...
    return cublasZtrsv_v2_64(handle, uplo, trans, diag, n, a, lda, x, incx);
  }
};
...
template <typename T>
static void fill_inputs(std::vector<T> &a, std::vector<T> &solution,
  ...
  for (int i = 0; i < n; ++i) {
    solution[i] =
        Ops<T>::value(0.25f * static_cast<float>(i + 1),
...
static void run_case(cublasHandle_t handle, const char *name,
  ...
  fill_inputs(a, solution, uplo, n, lda);
  std::vector<T> rhs = apply_op(a, solution, uplo, trans, diag, n, lda);
  ...
  if (use_64) {
    CHECK_CUBLAS(
        (Trsv<T>::call64(handle, uplo, trans, diag, n, d_a, lda, d_x, 1)));
  } else {
    CHECK_CUBLAS(
        (Trsv<T>::call32(handle, uplo, trans, diag, n, d_a, lda, d_x, 1)));
  }
...
int main() {
  ...
  run_type<float>(handle, "strsv", real_ops, 2);
  run_type<double>(handle, "dtrsv", real_ops, 2);
  run_type<cuComplex>(handle, "ctrsv", complex_ops, 3);
  run_type<cuDoubleComplex>(handle, "ztrsv", complex_ops, 3);
